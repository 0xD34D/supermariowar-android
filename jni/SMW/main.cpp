...
void DECLSPEC soundfinished(int channel)
{
	if(!g_PlayingSoundChannels[channel])
		LOGE("Error: SoundFinished() tried to clear a channel that was already cleared!\n");
	else
	{
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if(fp)
	{
		...
		{
			LOGW("Old options.bin detected.  Skipped reading it.\n");
		}
...
bool writeOptions( void )
{
	...
	if(fp)
	{
		...
		unsigned char abyte[35];
		abyte[0] = (char)game_values.spawnstyle;
		abyte[1] = (char)game_values.awardstyle;
		abyte[3] = (char)game_values.teamcollision;
		abyte[4] = (char)(game_values.screencrunch == true ? 1 : 0);
		abyte[5] = (char)(game_values.toplayer == true ? 1 : 0);
		abyte[6] = (char)game_values.scoreboardstyle;
		abyte[7] = (char)(game_values.teamcolors == true ? 1 : 0);
		abyte[8] = (char)(game_values.sound == true ? 1 : 0);
		abyte[9] = (char)(game_values.music == true ? 1 : 0);
		abyte[10] = (char)game_values.musicvolume;
		abyte[11] = (char)game_values.soundvolume;
		abyte[12] = (char)game_values.respawn;
		abyte[15] = (char)game_values.outofboundstime;
		abyte[16] = (char)game_values.cpudifficulty;
		abyte[19] = (char)game_values.framelimiter;
		abyte[20] = (char)game_values.bonuswheel;
		abyte[21] = (char)(game_values.keeppowerup == true ? 1 : 0);
		abyte[22] = (char)(game_values.showwinningcrown == true ? 1 : 0);
		abyte[23] = (char)(game_values.playnextmusic == true ? 1 : 0);
		abyte[24] = (char)game_values.pointspeed;
		abyte[25] = (char)game_values.swapstyle;
		abyte[28] = (char)game_values.overridepowerupsettings;
		abyte[29] = (char)(game_values.minigameunlocked == true ? 1 : 0);
		abyte[30] = (char)(game_values.startgamecountdown == true ? 1 : 0);
		abyte[31] = (char)(game_values.deadteamnotice == true ? 1 : 0);
		abyte[33] = (char)(game_values.tournamentcontrolstyle == true ? 1 : 0);
		abyte[34] = (char)(game_values.startmodedisplay == true ? 1 : 0);
		fwrite(abyte, sizeof(char), 35, fp);
		...
		{
			short iDevice;
			fwrite(&iDevice, sizeof(short), 1, fp);
		}
		...
		for(short iGameMode = 0; iGameMode < GAMEMODE_LAST; iGameMode++)
			fwrite(&(gamemodes[iGameMode]->goal), sizeof(short), 1, fp);

		fwrite(&game_values.gamemodemenusettings, sizeof(GameModeSettings), 1, fp);

		fwrite(&game_values.teamcounts, sizeof(short), 4, fp);
		fwrite(&game_values.teamids, sizeof(short), 12, fp);
