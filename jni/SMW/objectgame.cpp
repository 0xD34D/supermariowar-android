	...
	CObject(nspr, x, y)
{
	...
	col = x >> TILESHIFT;// / TILESIZE;
	row = y >> TILESHIFT;// / TILESIZE;
...
void MO_Yoshi::placeYoshi()
{
	...
	for(short tries = 0; tries < 64; tries++)
	{
		...
		short ixl = ix >> TILESHIFT;// / TILESIZE;
		short ixr = (ix + iw) >> TILESHIFT;// / TILESIZE;
		short iyt = iy >> TILESHIFT;// / TILESIZE;
		short iyb = (iy + ih) >> TILESHIFT;// / TILESIZE;
			...
			!g_map.block(ixl, iyt) && !g_map.block(ixr, iyt) && !g_map.block(ixl, iyb) && !g_map.block(ixr, iyb))
		{
			...
			short iDeathY = (iy+ih) >> TILESHIFT;///TILESIZE;
			short iDeathX1 = ix >> TILESHIFT;///TILESIZE;
			short iDeathX2 = (ix+iw) >> TILESHIFT;///TILESIZE;
			...
			while(iDeathY < MAPHEIGHT)
			{
				...
				{
					short top = ((iDeathY << 5) - ih) >> TILESHIFT;// / TILESIZE;
...
void MO_WalkingEnemy::update()
{
	...
	{		
		...
		else
		{
			...
			{
				probeFrontX >>= 5;//TILESIZE;
				probeCenterX >>= 5;//TILESIZE;
				probeY >>= 5;//TILESIZE;
...
void MO_SpinAttack::update()
{
	...
	if(owner)
	{
		...
		short iTop = iy >> TILESHIFT;// / TILESIZE;
		short iBottom = (iy + collisionHeight) >> TILESHIFT;// / TILESIZE;

		short iLeft;
		if(ix < 0)			
			iLeft = (ix + 640) >> TILESHIFT;// / TILESIZE;
		else
			iLeft = ix >> TILESHIFT;// / TILESIZE;

		short iRight = (ix + collisionWidth) >> TILESHIFT;// / TILESIZE;
