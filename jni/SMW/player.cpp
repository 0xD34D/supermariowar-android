				...
				if(!lockjump && !statue_timer && !fSuperStomp)
				{
					...
					{	//only if on the ground and the jump key was released somewhen after it was pressed the last time
						...
						if(playerKeys->game_down.fDown)
						{
							...
							short txl = ix >> TILESHIFT;// / TILESIZE;
							short txr;
							
							fPrecalculatedY = fy + vely;

							short ty = ((short)fPrecalculatedY + PH) >> TILESHIFT;// / TILESIZE;
							...
							if(ix + PW >= 640)
								txr = (ix + PW - 640) >> TILESHIFT;// / TILESIZE;
							else
								txr = (ix + PW) >> TILESHIFT;// / TILESIZE;
...
void CPlayer::chooseWarpExit()
{
	...
	if(exit->direction == 0)
	{
		...
		short iCol = ix >> TILESHIFT;// / TILESIZE;
		short iRow = iy >> TILESHIFT;// / TILESIZE;
...
void CPlayer::collision_detection_map()
{
	...
	short ty2 = ((short)fy + PH) >> TILESHIFT;// / TILESIZE;
	short tx = -1;
	...
	if(fy + PH >= 0.0f)
	{
		...
		{		//moving right
			if(fx + PW >= 640.0f)
			{
				tx = (short)(fx + PW - 640.0f) >> TILESHIFT;// / TILESIZE;
				fOldX -= 640.0f;
			}
			else
                tx = ((short)fx + PW) >> TILESHIFT;// / TILESIZE;
	...
	txl = iPlayerL >> TILESHIFT;// / TILESIZE;
	txc = iPlayerC >> TILESHIFT;// / TILESIZE;
	txr = iPlayerR >> TILESHIFT;// / TILESIZE;
	...
	{	//moving down / on ground
		ty = ((short)fPrecalculatedY + PH) >> TILESHIFT;// / TILESIZE;
...
bool CPlayer::collision_detection_checktop()
{
	...
	short ty = iy >> TILESHIFT;// / TILESIZE;
...
bool CPlayer::collision_detection_checkleft()
{
	...
	short tx = ix >> TILESHIFT;// / TILESIZE;
...
bool CPlayer::collision_detection_checkright()
{
	...
	if(ix + PW >= 640)
		tx = (ix + PW - 640) >> TILESHIFT;// / TILESIZE;
	else
		tx = (ix + PW) >> TILESHIFT;// / TILESIZE;
...
void CPlayer::collision_detection_checksides()
{
	...
	short txl = ix >> TILESHIFT;

	short txr = -1;
	if(ix + PW >= 640)
		txr = (ix + PW - 640) >> TILESHIFT;
	else
		txr = (ix + PW) >> TILESHIFT;

	short ty = iy >> TILESHIFT;
	short ty2 = (iy + PH) >> TILESHIFT;
