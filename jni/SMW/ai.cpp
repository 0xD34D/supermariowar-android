...
void CPlayerAI::Think(COutputControl * playerKeys)
{
	...
	***************************************************/
	...
	std::map<const int, AttentionObject*>::iterator itr = attentionObjects.begin();
	std::map<const int, AttentionObject*>::iterator lim = attentionObjects.end();
	while(itr != lim)
	{
		if(itr->second->iTimer > 0)
		{
			if(--(itr->second->iTimer) == 0)
			{
				...
				attentionObjects.erase((*itr).first);
	...
	***************************************************/
	...
	short iDeathY = iy >> TILESHIFT;// / TILESIZE;
	short iDeathX1 = ix >> TILESHIFT;// / TILESIZE;
	short iDeathX2;
	
	if(ix + PW >= 640)
		iDeathX2 = (ix + PW - 640) >> TILESHIFT;// / TILESIZE;
	else
		iDeathX2 = (ix + PW) >> TILESHIFT;// / TILESIZE;
