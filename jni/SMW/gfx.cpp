...
#else
	#define GFX_FLAGS	SDL_DOUBLEBUF | SDL_HWSURFACE
#endif
...
{
#ifdef ANDROID_NDK
	LOGI("Initializing video\n");
	fullscreen = true;
	if(SDL_Init(SDL_INIT_VIDEO) < 0)
	{
		LOGE( "Couldn't initialize SDL video subsystem: %s\n", SDL_GetError() );
		exit(1);
	}
	...
	if ( !screen )
	{
		LOGE( "Couldn't set %dx%d, %dbit video mode: %s\n", w, h, GFX_BPP, SDL_GetError() );
		exit(3);
	}
...
//	else
		screen = SDL_SetVideoMode(w, h, GFX_BPP, SDL_DOUBLEBUF | SDL_HWSURFACE);

    if ( screen == NULL ) 
	{
        LOGE("Couldn't set video mode %dx%d: %s\n", w, h, SDL_GetError());
		return false;
    }
#endif

    LOGI(" running @ %dx%d %dbpp (done)\n", w, h, screen->format->BitsPerPixel);
...
bool gfx_loadpalette()
{
	...
	if ( palette == NULL ) 
	{
        LOGE("Couldn't load color palette: %s\n", SDL_GetError());
		return false;
    }
...
bool gfx_loadimage(gfxSprite * gSprite, const std::string& f, bool fWrap, bool fUseAccel)
{
	...
	bool fRet = gSprite->init(f, 255, 0, 255, true);
...
bool gfx_loadimage(gfxSprite * gSprite, const std::string& f, Uint8 alpha, bool fWrap, bool fUseAccel)
{
	...
	bool fRet = gSprite->init(f, 255, 0, 255, alpha, true);
...
bool gfx_loadimage(gfxSprite * gSprite, const std::string& f, Uint8 r, Uint8 g, Uint8 b, bool fWrap, bool fUseAccel)
{
	...
	bool fRet = gSprite->init(f, r, g, b, true);
