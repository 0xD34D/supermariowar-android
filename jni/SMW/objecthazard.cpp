...
void MO_Explosion::update()
{
	...
	if(timer == 0)
	{
		...
		for(short iRow = 0; iRow < 5; iRow++)
		{
			...
			{
				short iTestRow = iTestY >> TILESHIFT;// / TILESIZE;
				for(short iCol = 0; iCol < 7; iCol++)
				{
					IO_Block * block = g_map.block(iTestX >> TILESHIFT/* / TILESIZE*/, iTestRow);
...
void MO_PirhanaPlant::KillPlant()
{
	...
	ifsoundonplay(sfx_kicksound);
	eyecandy[2].add(new EC_SingleAnimation(&spr_fireballexplosion, ix + (iDirection == 2 ? 0 : collisionWidth - 32), iy + (iDirection == 0 ? 0 : collisionHeight - 32), 3, 4));
