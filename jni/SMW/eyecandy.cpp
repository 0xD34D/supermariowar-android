	...
	EC_StillImage(nspr, (short)nx, (short)ny, iSrcOffsetX, 0, 32, 32)
{
	...
	if(ix + PWOFFSET < 0)
		tx = (ix + 640 + PWOFFSET) >> TILESHIFT;// / TILESIZE;
	else
		tx = (ix + PWOFFSET) >> TILESHIFT;// / TILESIZE;

	if(ix + PWOFFSET + PW >= 640)
		tx2 = (ix + PWOFFSET + PW - 640) >> TILESHIFT;// / TILESIZE;
	else
		tx2 = (ix + PWOFFSET + PW) >> TILESHIFT;// / TILESIZE;
}
...
void EC_Corpse::update()
{
	if(vely != 0.0f)
	{
		...
		if(nexty >= 0)
		{
			short ty = nexty >> TILESHIFT;// / TILESIZE;
