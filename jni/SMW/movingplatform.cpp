...
void MovingPlatform::update()
{
	...
	xf(pPath->dCurrentX[0]);
	yf(pPath->dCurrentY[0]);
}
	...
	*/
	...
	if(fColVelX > 0.01f || player->iHorizontalPlatformCollision == 3)
	{
		...
		if(coldec == collision_normal)
			...
			fRelativeX = player->fx + PW - fx + iHalfWidth - 640.0f;
		else
			fRelativeX = player->fx + PW - fx + iHalfWidth + 640.0f;
		...
		{
			short tx = (short)fRelativeX >> TILESHIFT;// / TILESIZE;
		
			int t1 = tile_flag_nonsolid;
			int t2 = tile_flag_nonsolid;

			if(fRelativeY1 >= 0.0f && fRelativeY1 < iHeight)
				t1 = iTileType[tx][(short)fRelativeY1 >> TILESHIFT/*/ TILESIZE*/].iFlags;

			if(fRelativeY2 >= 0.0f && fRelativeY2 < iHeight)
				t2 = iTileType[tx][(short)fRelativeY2 >> TILESHIFT/*/ TILESIZE*/].iFlags;
	...
	else if(fColVelX < -0.01f || player->iHorizontalPlatformCollision == 1)
	{
		...
		if(coldec == collision_normal)
			...
			fRelativeX = player->fx - fx + iHalfWidth - 640.0f;
		else
			fRelativeX = player->fx - fx + iHalfWidth + 640.0f;
	...
	{
		fRelativeX1 = player->fx - fx + iHalfWidth - 640.0f;
		fRelativeX2 = player->fx + PW - fx + iHalfWidth - 640.0f ;
	}
	else
	{
		fRelativeX1 = player->fx - fx + iHalfWidth + 640.0f;
	...
	if(fColVelY < 0.0f)
	{
		...
		{
			short ty = (short)fRelativeY >> TILESHIFT;// / TILESIZE;

			int t1 = tile_flag_nonsolid;
			int t2 = tile_flag_nonsolid;

			if(fRelativeX1 >= 0.0f && fRelativeX1 < iWidth)
				t1 = iTileType[(short)fRelativeX1 >> TILESHIFT/*/ TILESIZE*/][ty].iFlags;

			if(fRelativeX2 >= 0.0f && fRelativeX2 < iWidth)
				t2 = iTileType[(short)fRelativeX2 >> TILESHIFT/*/ TILESIZE*/][ty].iFlags;
	...
	else
	{
		...
		if(fRelativeY >= 0.0f && fRelativeY < iHeight)
		{
			short ty = (short)fRelativeY >>5;// / TILESIZE;
...
void MovingPlatform::check_map_collision_right(CPlayer * player)
{
	...
	if(player->fx + PW >= 640.0f)
		iTestBackgroundX = ((short)player->fx + PW - 640) >> TILESHIFT;// / TILESIZE;
	else
		iTestBackgroundX = ((short)player->fx + PW) >> TILESHIFT;// / TILESIZE;

	short iTestBackgroundY = ((short)player->fy + PH) >> TILESHIFT;// / TILESIZE;
	...
	short iTestBackgroundY2 = (short)player->fy >> TILESHIFT;// / TILESIZE;
...
void MovingPlatform::check_map_collision_left(CPlayer * player)
{
	...
	short iTestBackgroundX = (short)player->fx >> TILESHIFT;// / TILESIZE;
	short iTestBackgroundY = ((short)player->fy + PH) >> TILESHIFT;// / TILESIZE;
...
bool MovingPlatform::collision_detection_check_sides(IO_MovingObject * object)
{
	...
	if(fRelativeXLeft >= 0.0f && fRelativeXLeft < iWidth)
		txLeft = (short)fRelativeXLeft >> TILESHIFT;// / TILESIZE;

	if(fRelativeXRight >= 0.0f && fRelativeXRight < iWidth)
		txRight = (short)fRelativeXRight >> TILESHIFT;// / TILESIZE;

	short tyTop = -1;
	short tyBottom = -1;

	if(fRelativeYTop >= 0.0f && fRelativeYTop < iHeight)
		tyTop = (short)fRelativeYTop >> TILESHIFT;// / TILESIZE;

	if(fRelativeYBottom >= 0.0f && fRelativeYBottom < iHeight)
		tyBottom = (short)fRelativeYBottom >> TILESHIFT;// / TILESIZE;
...
void MovingPlatform::GetTileTypesFromPlayer(CPlayer * player, int * lefttile, int * righttile)
{
	...
	*righttile = tile_flag_nonsolid;
	...
	if(fRelativeX1 >= 0.0f && fRelativeX1 < iWidth)
		*lefttile = iTileType[(short)fRelativeX1 >> TILESHIFT/* / TILESIZE*/][ty].iFlags;

	if(fRelativeX2 >= 0.0f && fRelativeX2 < iWidth)
		*righttile = iTileType[(short)fRelativeX2 >> TILESHIFT/* / TILESIZE*/][ty].iFlags;
}
...
int MovingPlatform::GetTileTypeFromCoord(short x, short y)
{
	...
	return iTileType[(short)fRelativeX >> TILESHIFT/* / TILESIZE*/][(short)fRelativeY >> TILESHIFT/* / TILESIZE*/].iFlags;
}
...
void MovingPlatform::collide(IO_MovingObject * object)
{
	...
	if(fColVelX > 0.01f || object->iHorizontalPlatformCollision == 3)
	{
		float fRelativeX;
		if(coldec == collision_overlap_left)
			fRelativeX = object->fx + object->collisionWidth - fx + iHalfWidth - 640.0f;
		else if(coldec == collision_overlap_right)
			fRelativeX = object->fx + object->collisionWidth - fx + iHalfWidth + 640.0f;
		else
		...
		if(fRelativeX >= 0.0f && fRelativeX < iWidth)
		{
			...
			if((t1 & tile_flag_solid) || (t2 & tile_flag_solid))
			{
				...
				else
				{
					...
					//test to see if we put the object on top of a background tile
					short iTestBackgroundY = (short)object->fy >> TILESHIFT;// / TILESIZE;
					short iTestBackgroundY2 = ((short)object->fy + object->collisionHeight) >> TILESHIFT;// / TILESIZE;
					
					short iTestBackgroundX;
					if(object->fx + object->collisionWidth >= 640.0f)
						iTestBackgroundX = ((short)object->fx + object->collisionWidth - 640) >> TILESHIFT;// / TILESIZE;
					else
						iTestBackgroundX = ((short)object->fx + object->collisionWidth) >> TILESHIFT;// / TILESIZE;
	...
	else if(fColVelX < -0.01f || object->iHorizontalPlatformCollision == 1)
	{
		float fRelativeX;
		if(coldec == collision_overlap_left)
			fRelativeX = object->fx - fx + iHalfWidth - 640.0f;
		else if(coldec == collision_overlap_right)
			fRelativeX = object->fx - fx + iHalfWidth + 640.0f;
		else
		...
		if(fRelativeX >= 0.0f && fRelativeX < iWidth)
		{
			...
			if((t1 & tile_flag_solid) || (t2 & tile_flag_solid))
			{
				...
				else
				{
					...
					short iTestBackgroundY = (short)object->fy >> TILESHIFT;// / TILESIZE;
					short iTestBackgroundY2 = ((short)object->fy + object->collisionHeight) >> TILESHIFT;// / TILESIZE;
					short iTestBackgroundX = (short)object->fx >> TILESHIFT;// / TILESIZE;
...
bool MovingPlatform::IsInNoSpawnZone(short x, short y, short w, short h)
{
	...
	for(short x = 0; x < 2; x++)
	{
		for(short y = 0; y < 2; y++)
		{
			...
			{
				short tx = iRelativeX[x] >> TILESHIFT;// / TILESIZE;
				short ty = iRelativeY[y] >> TILESHIFT;// / TILESIZE;
