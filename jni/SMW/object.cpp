...
void CObject::GetCollisionBlocks(IO_Block * blocks[4])
{
	short xl = 0;
	if(ix < 0)
		xl = (ix + 640) >> TILESHIFT;// / TILESIZE;
	else
		xl = ix >> TILESHIFT;// / TILESIZE;

	short xr = 0;
	if(ix + iw >= 640)
		xr = (ix + iw - 640) >> TILESHIFT;// / TILESIZE;
	else
		xr = (ix + iw) >> TILESHIFT;// / TILESIZE;
	...
	if(iy >= 0 && iy < 480)
	{
		short yt = iy >> TILESHIFT;// / TILESIZE;
	...
	{
		short yb = (iy + ih) >> TILESHIFT;// / TILESIZE;
...
void IO_MovingObject::collision_detection_map()
{
	...
	//Could be optimized with bit shift >> TILESHIFT
	short ty = (short)fy >> TILESHIFT;// / TILESIZE;
	short ty2 = ((short)fy + collisionHeight) >> TILESHIFT;// / TILESIZE;
	short tx = -1;
	...
	if(fy + collisionHeight >= 0.0f)
	{
		...
		{		//moving right
			...
			{
				tx = (short)(fx + collisionWidth - 640.0f) >> TILESHIFT;// / TILESIZE;
				fOldX -= 640.0f;
			}
			else
                tx = ((short)fx + collisionWidth) >> TILESHIFT;// / TILESIZE;
		...
		{	//moving left
			tx = (short)fx >> TILESHIFT;// / TILESIZE;
	...
	txl = ix >> TILESHIFT;// / TILESIZE;

	if(ix + collisionWidth >= 640)
        txr = (ix + collisionWidth - 640) >> TILESHIFT;// / TILESIZE;
	else
		txr = (ix + collisionWidth) >> TILESHIFT;// / TILESIZE;
	...
	if(fMovingUp < -0.01f)
	{
		ty = (short)(fPrecalculatedY) >> TILESHIFT;// / TILESIZE;
	...
	{	//moving down / on ground
		ty = ((short)fPrecalculatedY + collisionHeight) >> TILESHIFT;// / TILESIZE;
...
bool IO_MovingObject::collision_detection_checksides()
{
	...
	short txl = -1, nofliptxl = ix >> TILESHIFT;
	if(ix < 0)
		txl = (ix + 640) >> TILESHIFT;
	else
		txl = nofliptxl;

	short txr = -1, nofliptxr = (ix + collisionWidth) >> TILESHIFT;
	if(ix + collisionWidth >= 640)
		txr = (ix + collisionWidth - 640) >> TILESHIFT;
	else
		txr = nofliptxr;

	short ty = iy >> TILESHIFT;
	short ty2 = (iy + collisionHeight) >> TILESHIFT;
